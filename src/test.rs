                    ...
                    while i < code.body.len() {
                        ...
                        if i >= 2 && matches!(code.body[i], Instr::Unary(UnaryOp::I32WrapI64)) {
                            if let (Instr::Local(LocalOp::Get, idx1), Instr::Local(LocalOp::Set, idx2)) = (&code.body[i - 1], &code.body[i+1]) {
                                ...
                                new_instrs.push(Instr::Unary(UnaryOp::I64ExtendI32S));
                    ...
                    while i < code.body.len() {
                        ...
                        if i >= 2 && matches!(code.body[i], Instr::Unary(UnaryOp::I32WrapI64)) {
                            if let (Instr::Local(LocalOp::Get, idx1), Instr::Local(LocalOp::Set, idx2)) = (&code.body[i - 1], &code.body[i+1]) {
                                ...
                                new_instrs.push(Instr::Unary(UnaryOp::I64ExtendI32U));
                                new_instrs.push(Instr::Local(LocalOp::Set, new_local2));
                                new_instrs.push(Instr::Local(LocalOp::Get, new_local2));
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx1));
                                // new_instrs.push(Instr::Binary(BinaryOp::I64Eq));
    ...
    // -------------------------------------f32.demote_f64, double2float----------------------------------------------
                        match(&code.body[i-2], &code.body[i-1], &code.body[i]){
                            ...
                            Instr::Local(LocalOp::Set, idx2)) => {
                                ...
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx1));
                                // new_instrs.push(Instr::Binary(BinaryOp::F64Eq));
                                new_instrs.push(Instr::Binary(BinaryOp::F64Eq));
                                new_instrs.push(Instr::Local(LocalOp::Set, new_local1));
                                // new_instrs.push(Instr::Block(FunctionType::new(&[], &[])));
                                new_instrs.push(Instr::Block(FunctionType::new(&[], &[])));
    ...
    if let Ok((mut module, _,_)) = result {
        ...
        for (_, func) in module.functions_mut() {
            ...
            if let Some(code) = func.code_mut() {
                ...
                while i < code.body.len() {
                    new_instrs.push(code.body[i].clone());
                    if (i == code.body.len() - 1) && (result_len > 0) {
                        //最后一条指令
                        if !matches!(code.body[i], Instr::Return)
                            && !matches!(code.body[i], Instr::Unreachable)
                        {
                            //如果最后一条指令不是return
...
pub fn implicit_integer_sign_change(input: &str, output: &str) {
    ...
    if let Ok((mut module, _, _)) = result {
            ...
            if let Some(cur_func_name) = &func.name.clone() {
                ...
                if let Some(code) = func.code_mut() {
                    ...
                    while i < code.body.len() {
                        ...
                        match (&code.body[i - 1], &code.body[i]) {
                            (
                                Instr::Load(LoadOp::I32Load8U, _idx),
                                Instr::Local(LocalOp::Set, idx),
                            ) => {
                                //let new_local1 = add_fresh_local(&mut code.locals, func_param_count, ValType::I32);
                                new_instrs.push(Instr::Block(FunctionType::new(&[], &[])));
                                ...
                                new_instrs.push(Instr::Binary(BinaryOp::I32Shl));
                                new_instrs.push(Instr::Const(Val::I32(24)));
                                new_instrs.push(Instr::Binary(BinaryOp::I32ShrS));
                                new_instrs.push(Instr::Const(Val::I32(0)));
                                // new_instrs.push(Instr::Binary(BinaryOp::I32GeS));
                                new_instrs.push(Instr::Binary(BinaryOp::I32GeS));
                                new_instrs.push(Instr::BrIf(Label::from(0 as usize))); // 如果小于0，跳转到unreachable
                                                                                       //new_instrs.push(Instr::Const(Val::I32(16))); // return 16
                                                                                       //new_instrs.push(Instr::Call(proc_exit_idx));
                                new_instrs.push(Instr::Unreachable); // 停止代码执行
                                new_instrs.push(Instr::End);
                                log_info!(
                                    "implicit_integer_sign_change:I32Load8U in {}",
                                    &func_name
                                );
                            }
                            (
                                Instr::Load(LoadOp::I32Load16U, _),
                                Instr::Local(LocalOp::Set, idx),
                            ) => {
                                new_instrs.push(Instr::Block(FunctionType::new(&[], &[])));
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx));
                                ...
                                new_instrs.push(Instr::End);
                                log_info!(
                                    "implicit_integer_sign_change:I32Load16U in {}",
                                    &func_name
                                );
                            }
                            (
                                Instr::Load(LoadOp::I64Load8U, _),
                                Instr::Local(LocalOp::Set, idx),
                            ) => {
                                new_instrs.push(Instr::Block(FunctionType::new(&[], &[])));
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx));
                                ...
                                new_instrs.push(Instr::Const(Val::I64(56)));
                                new_instrs.push(Instr::Binary(BinaryOp::I64ShrS));
                                new_instrs.push(Instr::Const(Val::I64(0)));
                                new_instrs.push(Instr::Binary(BinaryOp::I64GeS));
                                new_instrs.push(Instr::BrIf(Label::from(0 as usize))); // 如果小于0，跳转到unreachable
                                                                                       //new_instrs.push(Instr::Const(Val::I32(16))); // return 16
                                                                                       //new_instrs.push(Instr::Call(proc_exit_idx));
                                new_instrs.push(Instr::Unreachable); // 停止代码执行
                                new_instrs.push(Instr::End);
                                log_info!(
                                    "implicit_integer_sign_change:I64Load8U in {}",
                                    &func_name
                                );
                            }
                            (
                                Instr::Load(LoadOp::I64Load16U, _),
                                Instr::Local(LocalOp::Set, idx),
                            ) => {
                                new_instrs.push(Instr::Block(FunctionType::new(&[], &[])));
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx));
                                ...
                                new_instrs.push(Instr::End);
                                log_info!(
                                    "implicit_integer_sign_change:I64Load16U in {}",
                                    &func_name
                                );
                            }
                            (
                                Instr::Load(LoadOp::I64Load32U, _),
                                Instr::Local(LocalOp::Set, idx),
                            ) => {
                                new_instrs.push(Instr::Block(FunctionType::new(&[], &[])));
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx));
                                ...
                                new_instrs.push(Instr::End);
                                log_info!(
                                    "implicit_integer_sign_change:I64Load32U in {}",
                                    &func_name
                                );
                            }
                            _ => {}
        ...
        if let Some((_, malloc)) = find_func_by_name("dlmalloc", module.functions_mut()) {
            // 2:for functions
            let malloc_p0 = get_param_n_idx(malloc, 0);
            insert_prefix(
                malloc,
                vec![
                    Instr::Block(FunctionType::new(&[], &[])),
                    Instr::Local(LocalOp::Get, malloc_p0),
                    Instr::Const(Val::I32(0)),
                    Instr::Binary(BinaryOp::I32GtS),
                    Instr::BrIf(Label::from(0 as usize)), // if *malloc_p0 > 0, get out.
        ...
        if let Some((_, malloc)) = find_func_by_name("__memcpy", module.functions_mut()) {
            // 2:for functions
        ...
        if let Some((_, malloc)) = find_func_by_name("memmove", module.functions_mut()) {
            // 2:for functions
            let malloc_p2 = get_param_n_idx(malloc, 2);
            insert_prefix(
                malloc,
        ...
        if let Some((_, malloc)) = find_func_by_name("strncpy", module.functions_mut()) {
            // 2:for functions
            let malloc_p2 = get_param_n_idx(malloc, 2);
            insert_prefix(
                malloc,
                    ...
                    while i < code.body.len() {
                        ...
                        match (&code.body[i - 2], &code.body[i - 1], &code.body[i]) {
                            (
                                ...
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx1));
                                new_instrs.push(Instr::Const(Val::I32(0)));
                                new_instrs.push(Instr::Binary(BinaryOp::I32LtS)); // if b < 0 , err
                                new_instrs.push(Instr::BrIf(Label::from(0 as usize)));

                                new_instrs.push(Instr::Local(LocalOp::Get, *idx2));
                                new_instrs.push(Instr::Const(Val::I32(32)));
                                new_instrs.push(Instr::Binary(BinaryOp::I32GeS)); // if a > 32, err
                                new_instrs.push(Instr::BrIf(Label::from(0 as usize)));

                                new_instrs.push(Instr::Local(LocalOp::Get, *idx2));
                                new_instrs.push(Instr::Const(Val::I32(0)));
                                new_instrs.push(Instr::Binary(BinaryOp::I32LtS)); // if a < 0, err
                                new_instrs.push(Instr::BrIf(Label::from(0 as usize)));
                                new_instrs.push(Instr::Br(Label::from(1 as usize)));
                                new_instrs.push(Instr::End);
                            ...
                            (
                                ...
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx1));
                                new_instrs.push(Instr::Const(Val::I64(0)));
                                new_instrs.push(Instr::Binary(BinaryOp::I64LtS));
                                new_instrs.push(Instr::BrIf(Label::from(0 as usize)));
                    ...
                    while i < code.body.len() {
                        ...
                        match (
                            ...
                            (
                                ...
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx1));
                                new_instrs.push(Instr::Binary(BinaryOp::I32GeU)); // 若sum >= x, 则直接跳出
                                new_instrs.push(Instr::BrIf(Label::from(0 as usize)));
                            ...
                            (
                                ...
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx3));
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx1));
                                new_instrs.push(Instr::Binary(BinaryOp::I64GeU)); // 若sum >= x, 则直接跳出
                                new_instrs.push(Instr::BrIf(Label::from(0 as usize)));
                                //new_instrs.push(Instr::Const(Val::I32(18))); // return 18
                                //new_instrs.push(Instr::Call(proc_exit_idx));
                                new_instrs.push(Instr::Unreachable);
                            ...
                            (
                                ...
                                Instr::Local(LocalOp::Set, _idx3),
                                 ...
                                 /* new_instrs.push(Instr::Block(FunctionType::new(&[], &[])));
                                     ...
                                     new_instrs.push(Instr::Binary(BinaryOp::I32GeU)); // 若 b >= a, 则直接跳出
                                     new_instrs.push(Instr::BrIf(Label::from(0 as usize)));
                                     //new_instrs.push(Instr::Const(Val::I32(18))); // return 18
                                     //new_instrs.push(Instr::Call(proc_exit_idx));
                                     new_instrs.push(Instr::Unreachable);
                            ...
                            (
                                ...
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx1)); // b
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx2)); // a (sub = b - a)
                                new_instrs.push(Instr::Binary(BinaryOp::I64GeU)); // 若 b >= a, 则直接跳出
                                new_instrs.push(Instr::BrIf(Label::from(0 as usize)));
                                //new_instrs.push(Instr::Const(Val::I32(18))); // return 18
                                //new_instrs.push(Instr::Call(proc_exit_idx));
                                new_instrs.push(Instr::Unreachable);
                            ...
                            (
                                ...
                                new_instrs.push(Instr::Block(FunctionType::new(&[], &[])));
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx1)); // b
                                new_instrs.push(Instr::Unary(UnaryOp::I64ExtendI32U)); // 扩展为无符号64位整数
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx2)); // a
                                new_instrs.push(Instr::Unary(UnaryOp::I64ExtendI32U)); // 扩展为无符号64位整数
                                new_instrs.push(Instr::Binary(BinaryOp::I64Mul));
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx3));
                                new_instrs.push(Instr::Unary(UnaryOp::I64ExtendI32U)); // 扩展为无符号64位整数
                                new_instrs.push(Instr::Binary(BinaryOp::I64Eq));
                                new_instrs.push(Instr::BrIf(Label::from(0 as usize)));
                    ...
                    while i < code.body.len() {
                        ...
                        match (&code.body[i - 1], &code.body[i], &code.body[i + 1]) {
                            (
                                ...
                                new_instrs.push(Instr::Const(Val::F32(0.0.into())));
                                new_instrs.push(Instr::Local(LocalOp::Get, *idx2));
                                new_instrs.push(Instr::Binary(BinaryOp::F32Ne));
                                new_instrs.push(Instr::Local(LocalOp::Set, new_local1));
                                new_instrs.push(Instr::Block(FunctionType::new(&[], &[])));
                                new_instrs.push(Instr::Local(LocalOp::Get, new_local1));
                                new_instrs.push(Instr::BrIf(Label::from(0 as usize))); // 没有溢出，跳出
    ...
    if let Ok((mut module, _, _)) = result {
        ...
        insert_prefix(
            ...
            vec![
                ...
                Instr::BrIf(Label::from(0 as usize)),
                Instr::Local(LocalOp::Get, malloc_p0),
        ...
        let get_malloc_postfix = |func: &mut Function,
         ...
         -> Vec<Instr> {
            ...
            let mut postfix = vec![
                ...
                Instr::Block(FunctionType::new(&[ValType::I32], &[ValType::I32])),
        ...
        match calloc {
            Some((_, calloc)) => {
                ...
                let calloc_prefix = vec![
                    ...
                    Instr::Unary(UnaryOp::I64ExtendI32U),
                    Instr::Local(LocalOp::Get, calloc_p1),
                    Instr::Unary(UnaryOp::I64ExtendI32U),
                    Instr::Binary(BinaryOp::I64Mul),
                    Instr::Const(Val::I64(0xFFFFFFFF)),
                    Instr::Binary(BinaryOp::I64GeU),
...
fn returns_to_outer_block_jmp(f: &mut Function) {
    ...
    if let Some(code) = f.code_mut() {
        ...
        for r_loc in return_locs {
            ...
            instrs[r_loc.idx] = Instr::Br(Label::from(r_loc.level as u32));
        }

        // insert the block wrapper around the function body.
        // instrs.insert(0, Instr::Block(BlockType(return_type)));
        if let Some(return_type) = return_type {
            // instrs.insert(0, Instr::Block(FunctionType::new(&[return_type], &[])));
            instrs.insert(0, Instr::Block(FunctionType::new(&[], &[return_type])));
        } else {
            instrs.insert(0, Instr::Block(FunctionType::new(&[], &[])));
        }
